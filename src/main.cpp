...
int main(int argc, char* argv[]) {
    ...
    std::ifstream* source_file = new std::ifstream(argv[1], std::ios::in);
...
void parse(std::ifstream* const source_file) {
    ParseContext* const cx = new ParseContext();
...
void parse_number(ParseContext* const cx) {
    if ((cx->next >= '0' && cx->next <= '9') || cx->next == '_') {
        ...
        // End number.
        Value* value = Num::from_string(cx->token);
...
Value::ValueId call_function(ParseContext* const cx) {
    ...
    Value* value = (function_map.count(cx->token) > 0
        ? function_map[cx->token](cx)
        : new Error("Invalid function \"" + cx->token + "\" called")
    );
